...
while [[ $MONTH -lt 13 ]] ; do
	...
	pr_count=0
	gh pr list --state=all --limit=1000 --search "is:pr label:\"External Contribution\"  created:${START_DATE}..${END_DATE}" --json createdAt,closedAt | jq -r '.[] | [.createdAt, .closedAt] | @tsv' > prs.tsv
