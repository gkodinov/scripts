...
while [[ $YEAR -le $END_YEAR ]]; do
  ...
  while [[ $MONTH -le $END_MONTH ]] ; do
          ...
          pr_count=0
	  gh pr list --repo=MariaDB/server --state=all --limit=1000 --search "is:pr label:\"External Contribution\"  created:${START_DATE}..${END_DATE}" --json createdAt,closedAt | jq -r '.[] | [.createdAt, .closedAt] | @tsv' > prs.tsv
